...
def get_capacitor_equipments() -> list[CapacitorEquipment]:
    ...
    logger.info("parsing capacitor equipment...")
    ...
    while flag > 0:
        ...
        if str(model_dict) not in capacitor_equipments_catalog:
            capacitor_equipments_catalog[str(model_dict)] = capacitor_equipment

        flag = odd.Capacitors.Next()
    return capacitor_equipments_catalog


def get_capacitors(system: System, catalog: dict[str, Component]) -> list[DistributionCapacitor]:
    ...
    while flag > 0:
        ...
        equipment_from_libray = get_equipment_from_system(
            capacitor_equipment, CapacitorEquipment, catalog
        )
