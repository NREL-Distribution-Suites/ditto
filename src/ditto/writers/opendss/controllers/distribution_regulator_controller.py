...
class RegulatorControllerMapper(OpenDSSMapper):
    ...
    def map_name(self):
        ...
        self.opendss_dict["Transformer"] = self.xfmr_name.replace(" ", "_").replace(".", "_")
    ...
    def map_controlled_bus(self):
        self.opendss_dict[
            "Bus"
        ] = f"{self.model.controlled_bus.name.replace(' ', '_').replace('.', '_')}{self.phase_map[self.model.controlled_phase]}"
